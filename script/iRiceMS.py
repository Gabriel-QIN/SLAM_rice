...
def feature_extraction(seq, fusion=True):
    feature_total = []
    seq = seq.strip()
    ...
    if fusion:
        PCP9_feature = PCP9().main(seq)
        feature_total.extend(PCP9_feature)
        feature_PWAA = PWAA().main(seq)
        feature_total.extend(feature_PWAA)
        feature_ARPC = ARPC().main(seq)
        feature_total.extend(feature_ARPC)
        feature_1_EBGW = EBGW(1).main(seq)
        feature_2_EBGW = EBGW(2).main(seq)
        feature_3_EBGW = EBGW(3).main(seq)
        feature_4_EBGW = EBGW(4).main(seq)
        feature_5_EBGW = EBGW(5).main(seq)
        ...
        feature_CTD = CTD().main(seq)
        feature_total.extend(feature_CTD)
        feature_CC = CC().main(seq)
        feature_total.extend(feature_CC)
...
if __name__ == '__main__':
    ...
    result_list = []
    fea = 'fusion'
    desc=['PTM', 'Feature', 'Acc', 'th','Rec/Sn', 'Pre', 'F1', 'Spe', 'MCC', 'AUROC', 'AUPRC', 'TN', 'FP', 'FN', 'TP']
    for ptm in ['kac', 'kcr', 'khib', 'kmal', 'ksucc', 'kla']:
        train_fa = f'win51/{ptm}_train_ratio_{ratio}.fa'
        test_fa = f'win51/{ptm}_test_ratio_{ratio}.fa'
        train_fa = [[record.id,str(record.seq)] for record in SeqIO.parse(train_fa, "fasta")]
        test_fa = [[record.id,str(record.seq)] for record in SeqIO.parse(test_fa, "fasta")]
        sequenceLength = 51  # 提供序列的总长度
        train_X = []
        train_Y = []
        for desc,seq in train_fa:
            tmp = desc.split('|')
            label = float(tmp[1])
            seq = seq.strip()
            feature_total = feature_extraction(seq)
            train_X.append(feature_total)
            train_Y.append(label)
        test_X = []
        test_Y = []
        for desc,seq in test_fa:
            tmp = desc.split('|')
            label = float(tmp[1])
            seq = seq.strip()
            feature_total = feature_extraction(seq)
            test_X.append(feature_total)
            test_Y.append(label)
        ...
        df = pd.DataFrame([test_y, preds]).transpose()
        df.columns = ['Label', 'Pred']
        df.to_csv(f'{ptm}_ratio_{ratio}_{fea}.csv')
        acc_, th_, rec_, pre_, f1_, spe_, mcc_, auc_, pred_class, auprc_, tn, fp, fn, tp = eval_metrics(preds, test_y)
        result_info = [ptm, fea, (tn+tp)/(tn+tp+fp+fn), th_, rec_, pre_, f1_, spe_, mcc_, auc_, auprc_, tn, fp, fn, tp]
        result_list.append(result_info)
        print_results(result_info, desc)


    df = pd.DataFrame(result_list)
    df.to_csv(f'fusion_results_ratio_{ratio}.csv', index=False)
