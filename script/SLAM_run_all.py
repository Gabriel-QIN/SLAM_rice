...
if __name__=='__main__':
    ...
    for ptm in ptm_type:
        ...
        if args.transfer:
            model.load_state_dict(torch.load(f'PLA_ratio_{ratio}_pretrained.pt'))
        ...
        best_epoch = 0
        patience = 0
        max_patience = args.patience # patience_dict[ptm]

        desc=['PTM', 'Epoch', 'Acc', 'th','Rec/Sn', 'Pre', 'F1', 'Spe', 'MCC', 'AUROC', 'AUPRC', 'TN', 'FP', 'FN', 'TP']
        for epoch in range(num_epochs):
            ...
            result_info = [ptm, epoch, (tn+tp)/(tn+tp+fp+fn), th_, rec_, pre_, f1_, spe_, mcc_, auc_, auprc_, tn, fp, fn, tp]
            ...
            if auc_ >= best_auc:
                ...
                if save_model:
                    save_path = osp.join(model_dir, f'best_{ptm}_model_epoch.pt')
                    torch.save(model.state_dict(), save_path)
        print_results(best_result, desc)

        with open(osp.join(result_dir, f'{ptm}_loss_curve.txt'), 'w') as f:
            for l in loss_list:
                f.write(f'{l[0]}\t{l[1]}\n')
        # Save training step loss
        loss_df = pd.DataFrame(all_train_loss_list)
        loss_df.columns = ['Loss']
        loss_df.to_csv(osp.join(result_dir, f'{ptm}_all_train_step_loss.txt'), sep='\t')
        ...
        logit_df.to_csv(osp.join(result_dir, f"{ptm}_logits_results.txt"), sep='\t', index=False)

        epoch_df = pd.DataFrame(result_list)
        epoch_df.columns = desc
        epoch_df.to_csv(osp.join(result_dir, f'{ptm}_epoch_result.csv'), sep='\t', index=False)

        epoch_df = pd.DataFrame([best_result])
        epoch_df.columns = desc
        epoch_df.to_csv(osp.join(result_dir, f'{ptm}_best_result.csv'), sep='\t', index=False)
        perf_data.append(best_result)
    perf_df = pd.DataFrame(perf_data)
    perf_df.columns = desc
    perf_df.to_csv(osp.join(result_dir, f'final_performance_{args.ptm_type}_{args.project_name}.csv'), sep='\t', index=False)
