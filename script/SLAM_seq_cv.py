...
def arg_parse():
    ...
    parser.add_argument('--ratio', type=int, default=1, metavar='[Int]',
                        help='Negative sample ratio. (0,1,2,3 for neg:pos = all/1/2/3:1)')          
    parser.add_argument("--model", default='Models/SLAM_rice', type=str,
                        help="Directory for model storage and logits. Default:`Models/SLAM_rice`.")
    parser.add_argument("--result", default='RicePTM/CV', type=str,
    ...
    parser.add_argument('--transfer', action="store_true", help='Whether to use pretrained model.')
...
if __name__=='__main__':
    ...
    for ptm in args.ptm_type.split(','):
        ...
        for fold, data in enumerate(k_fold_list):
            ...
            train_ds = SLAMDatasetSeq(train_list, tokenizer, feature=manual_fea)
            ...
            print(f"Fold {fold} | Training datadset: {len(train_ds)}   |Testing dataset: {len(valid_ds)}")
            model = SLAMNetSeq(BERT_encoder=None, vocab_size=tokenizer.vocab_size, encoder_list=encoder_list,win_size=window_size,embedding_dim=32, fea_dim=41, hidden_dim=64, out_dim=32, n_layers=n_layers,dropout=dropout,bidirectional=bidirectional,kernel_size=kernel_size).to(device)
            ...
            desc=['PTM', 'Fold', 'Acc', 'th','Rec/Sn', 'Pre', 'F1', 'Spe', 'MCC', 'AUROC', 'AUPRC', 'TN', 'FP', 'FN', 'TP']
            for epoch in range(num_epochs):
                ...
                result_info = [project, fold, (tn+tp)/(tn+tp+fp+fn), th_, rec_, pre_, f1_, spe_, mcc_, auc_, auprc_, tn, fp, fn, tp]
                ...
                if auc_ >= best_auc:
                    ...
                    if save_model:
                        save_path = osp.join(model_dir, f'best_{project}_model_fold{fold}.pt')
                        torch.save(model.state_dict(), save_path)
            print(f'\n{ptm}|Fold{fold} best result:\n')
            print_results(best_result, desc)
            all_results.append(best_result)
    final = pd.DataFrame(all_results)
    final.columns = desc
    final.to_csv(osp.join(result_dir, f'{project}_fold_result_cv.csv'), sep='\t', index=False)
    # print_results(best_result, desc)
