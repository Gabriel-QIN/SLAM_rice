...
def get_geo_feat(X, edge_index, D_count=16):
    """
...
def _get_distance(X, edge_index, D_count=16):
...
def get_graph_fea(pdb_path, pos, nneighbor=32, radius=10, atom_type='CA', cal_cb=True, D_count=16):
    ...
    node, edge = get_geo_feat(X, edge_index, D_count)
    return Data(x=node, edge_index=edge_index, edge_attr=edge, name=os.path.basename(pdb_path).split('.')[0])

class SLAMDataset(object):
    def __init__(self, sample_file, tokenizer, pdb_dir=None, feature=None, nneighbor=32, atom_type='CA', D_count=1):
        self.seq_list = []
        self.label_list = []
        self.feature_list = []
        self.pdb_list = []
        ...
        for record in tqdm(seqlist):
            ...
            if not os.path.exists(pdb_path):
                ...
                data = get_graph_fea(pdb_path, pos, nneighbor=nneighbor, atom_type=atom_type, cal_cb=True, D_count=D_count)
...
def arg_parse():
    ...
    parser.add_argument("--project_name", default='SLAM', type=str,
                            help="Project name for saving model checkpoints and best model. Default:`SLAM`.")
    parser.add_argument("--train", default=osp.join(root_dir, 'general_train_ratio_all.fa'), type=str,
                            help="Data directory. Default:`'Datasets/general_train_ratio_all.fa'`.")
    parser.add_argument("--test", default=osp.join(root_dir, 'general_test_ratio_all.fa'), type=str,
                            help="Data directory. Default:`'Datasets/general_test_ratio_all.fa'`.")
    parser.add_argument("--model", default='/mnt/data2024/zhqin/project/SLAM/Model', type=str,
    ...
    parser.add_argument('--rbfdim', type=int, default=16, metavar='[Int]',
                        help='RBF dimensions. (default:16)')
    ...
    parser.add_argument('--encoder', type=str, default='cnn,lstm,fea,plm,gnn', metavar='[Str]',
                        help='Encoder list separated by comma chosen from cnn,lstm,fea,plm,gnn. (default:`cnn,lstm,fea,plm,gnn`)')
    ...
    parser.add_argument('--nneighbor', type=int, default=64, metavar='[Int]', help='Maximal number of neighbors.')
...
if __name__=='__main__':
    ...
    node_dim = 12 + 15 + D_count * 15 #267
    edge_dim = 16 + 36 + 4 + D_count * 36 # 632
    ...
    print(train_file)
    train_ds = SLAMDataset(train_file, tokenizer, pdb_dir=pdb_dir, feature=manual_fea, nneighbor=nneighbor, atom_type=atom_type, D_count=D_count)
    test_file = args.test
    test_ds = SLAMDataset(test_file, tokenizer, pdb_dir=pdb_dir, feature=manual_fea, nneighbor=nneighbor, atom_type=atom_type, D_count=D_count)
    ...
    for epoch in range(num_epochs):
        ...
        if valid_loss < best_loss:
            ...
            if save_model:
                save_path = osp.join(model_dir, f'best_{project}_model_epoch_{epoch}.pt')
