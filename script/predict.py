...
if __name__=='__main__':
    ...
    print(pred_df)
    all_df.to_csv('Results_all.csv', sep='\t', index=False)
    pred_df.to_csv('Results_pred.csv', sep='\t', index=False)
    ptm_list = [p.replace('k','K') for p in ptm_types.split(',')]
    draw_pie(all_df, ptm_list, 'pie.png')
    draw_bar(all_df, ptm_list,  'bar.png')